...
<div className="explore-menu position-relative">
    <h1 className="d-flex align-items-center justify-content-between">
        ...
        <i className='bi bi-arrow-left-circle scroll-icon' onClick={scrrollLeft}></i>
        <i className='bi bi-arrow-right-circle scroll-icon' onClick={scrrolRight}></i>

    </div>
    </h1>
    <p>Explore cureted lists of dishes from top categories</p>
        ...
        categories.map((item,index)=>{
          return(
            <div key={index} className="text-center explore-menu-list-item" onClick={()=>setCategory(prev=>prev===item.category? 'All':item.category)}>
              ...
              <p className='mt-2 fw-bold text-primary'>{item.category}</p>
            </div>
          )
        })
      }
